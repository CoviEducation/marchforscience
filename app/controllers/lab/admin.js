...
export default Ember.Controller.extend({
  ...
  actions: {
    ...
    removeTeammate(adminId, adminObj){
      $.ajax(`/api/LabMembership/${adminId}`, {
        method: 'DELETE'
    ...
    addTeammate(email, title) {
      ...
      .then((data=[], status, res) => {
        ...
        return $.ajax(`/api/LabMembership`, {
          method: 'POST',
          data: {
            "title": title,
            "labId": this.get('model.id'),
            "userId": data[0].id
          }
        })
      ...
      .then ((data={}, status, res) => {
        ...
        this.set('model.admins', admins.slice());
        return this.get('notifications').success('Lab admin added.');
      }, (err) => {
  ...
  saveEndorsements (){
    return $.ajax(`/api/labs/${get(this.model, 'id')}`, {
      method: 'PATCH',
      data: {
